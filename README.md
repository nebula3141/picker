...
- **Works on potato PCs** — decodes at 25% resolution by default. A 60MP Canon R5 RAW file that makes Lightroom swap to disk? PICker shows it instantly. Bump to 100% only when you need to pixel-peep.
...
Download `PICker-4.6.0-setup.exe` from [Releases](https://github.com/nebula3141/picker/releases). Includes everything — Python runtime, Qt, ffmpeg for video thumbnails. One click install.

### Standalone exe

Grab `PICker-4.6.0.exe` from [Releases](https://github.com/nebula3141/picker/releases). Single file, no install. Drop it anywhere and run.
...
- **Keyboard-driven** — `←/→` navigate, `Z` for 1:1 focus check, `R` rotate, `C` crop, `O` open in Photoshop/Lightroom.
...
|---------|---------|-------------|
| Resolution % | 25% | Decode scale for slideshow. 25% on a 60MP file = 15MP — still sharp on a 4K display, but uses 1/16th the RAM. |
...
Output: `dist/PICker-4.6.0.exe`. Drop `ffmpeg.exe` / `ffprobe.exe` next to spec file before building to bundle them.
...
Produces `installer_output/PICker-4.6.0-setup.exe` with Start Menu shortcut, file associations, context menus, and bundled ffmpeg.
